...
class KMeans:
    ...
    def cluster(self, data):
        center_indexes = self._find_random_means(len(data))
        ...
        while iteration_counter != self.max_iterations:
            ...
            if np.all(temp_center_indexes == center_indexes):
                print(f'after {iteration_counter} iteration(s)')
                print(f"converged because {temp_center_indexes} = {center_indexes}.")
                return
            center_indexes = temp_center_indexes

    def _find_random_means(self, count_of_data_points):
